...
/**
 * isCommandChain - test if the current
 ...
 * Return: Void
 */
void checkCommandChain(custom_info_t *info,
...
/**
 * replaceCustomAlias - replaces aliases in the tokenized string
 * @info: the parameter struct
 *
...
/**
 * replaceCustomVariables - replaces
...
int replaceCustomVariables(custom_info_t *info)
{
	...
	{
	if (info->argumentVector[i][0] != '$' ||
			!info->argumentVector[i][1])
	continue;

	if (!customStringCompare(info->argumentVector[i], "$?"))
	{
	replaceCustomString(&(info->argumentVector[i]),
			customStringDuplicate(convertInteger
				(info->executionStatus, 10, 0)));
	continue;
	}
	if (!customStringCompare(info->argumentVector[i], "$$"))
	{
	replaceCustomString(&(info->argumentVector[i]),
			customStringDuplicate
			(convertInteger(getpid(), 10, 0)));
	continue;
	}
	node = nodeStartsWith(info->environment,
			&info->argumentVector[i][1], '=');
	if (node)
	{
	replaceCustomString(&(info->argumentVector[i]),
			customStringDuplicate(customStringCharacter
				(node->string, '=') + 1));
	continue;
	}
	replaceCustomString(&info->argumentVector[i],
...
/**
 * replaceCustomString - replaces a string
