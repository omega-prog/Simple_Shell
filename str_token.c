...
/**
 * customStringToWords - splits a string into
 * words. Repeat delimiters are ignored
...
char **customStringToWords(char *str, char *delimiters)
{
	...
	for (i = 0; str[i] != '\0'; i++)
		if (!isDelimiterCharacter(str[i],
					delimiters) && (isDelimiterCharacter(str[i + 1],
						delimiters) || !str[i + 1]))
			numWords++;
...
/**
 * customStringToWords2 - splits a string into words
 * @str: the input string
