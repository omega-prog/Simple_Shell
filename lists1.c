...
/**
 * listToStrings - returns an array of strings from the list->str
 * @list: pointer to the first node
 *
...
/**
 * printCustomList - prints all elements of a custom_list_t linked list
 * @list: pointer to the first node
 *
 * Return: size of the list
 */
size_t printCustomList(const custom_list_t *list)
{
	...
	while (list)
	{
	customPuts(convertInteger(list->number, 10, 0));
	customPutchar(':');
	customPutchar(' ');
	customPuts(list->string ? list->string : "(nil)");
	customPuts("\n");
	list = list->next;
	count++;
	}
...
/**
 * getCustomNodeIndex - gets the index of a node
 * @list: pointer to the list head
